...
exports.get_settings = function(req, res, next) {
    ...
    let get_cve_info = function (callback) {
        const args = {
                    ...
                    if(cveDatasource==='Network Profiler')
{
    ...
    let get_device_info=function(callback){
        var deviceTable=[];
        var t=0;
    ...
    try{
        ...
        const $ = cheerio.load(data);
                        ...
                        console.log('i:'+t+'devices:'+deviceList)
                        t++;
    ...
    async.parallel([get_cve_info,get_device_info],function (err,data) {
        if (err) {
